...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        self.mark_safe(cell)
        cell_neighbors = set()
        for offi in range(-1, 2):
            for offj in range(-1, 2):
        ...
        if count == 0:
            ...
            s = Sentence(cell_neighbors, count)
        ...
        knowledge_copy = deepcopy(self.knowledge)
        for set_ in knowledge_copy:
            for subset in knowledge_copy:
                if set_ != subset:
                    if subset.cells.issubset(set_.cells):
                            new_sentence = Sentence(set_.cells - subset.cells, set_.count - subset.count)
                            ...
                            if set_ in self.knowledge:
                                self.knowledge.remove(set_)
